...
import { getExistingShapes } from "@/lib/get-existing-shapes";

type Objects = "rectangle" | "circle";

export interface ExistingShapes {
    shape: Objects;
    startingX: number;
    startingY: number;
    endingX: number;
...
const Canvas = () => {
    ...
    const { connected, data, error, sendMessage } = useWebsocket("ws://localhost:8080");
    const [shape, setShape] = useState<Objects>("rectangle");
        ...
        if (canvas.current && connected) {
            ...
            canvas.current.addEventListener("mousemove", (e) => {
                if (mouseDown.current) {
                    const { x, y } = getMousePosition(canvas.current!, e)
                    showRectangle(startingX.current, startingY.current, x - startingX.current, y - startingY.current, canvas.current!, existingShapes.current)
                }
            })

            canvas.current.addEventListener("mouseup", (e) => {
                const { x, y } = getMousePosition(canvas.current!, e)
                makeRectangle(startingX.current, startingY.current, x - startingX.current, y - startingY.current, canvas.current!)
        ...
        <div className="relative">
            ...
            <div className="absolute bg-[#232329] w-[50vw] translate-x-[50%] h-[50px] top-4 rounded-lg flex items-center gap-1 px-1 py-1">
                <div onClick={() => {
                    ...
                    shape === 'rectangle' ? "bg-[#403E6A]" : "active:border active:border-[#B1AEFF]"
                )}>
                    <Square className="text-white w-5 h-5" />
                    <p className="text-neutral-400 text-[10px] absolute self-end pb-[5px] left-[34px]">1</p>
                </div>
                <div onClick={() => {
                    setShape("circle")
                }} className={cn(
                    "flex items-center gap-px h-full cursor-pointer bg-[#31303B] px-3 rounded-lg relative border border-[#31303B]",
                    shape === 'circle' ? "bg-[#403E6A]" : "active:border active:border-[#B1AEFF]"
                )}>
                    <Circle className="text-white w-5 h-5" />
                    <p className="text-neutral-400 text-[10px] absolute self-end pb-[5px] left-[34px]">2</p>
                </div>
            </div>
        </div>
        </>
